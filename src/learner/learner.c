...
void find_best_rule(corpus_t corpus){
    ...
    for(int i = 0; i < errors->length; i++){
		...
		pattern.prevtag2, pattern.prevtag1, pattern.nexttag1, pattern.nexttag2, pattern.nexttag3);
...
//    }
    ...
    //printf("made it past error ordering\n");
    //for(int i = 0; i < count; i++){
	//errors_ordered[i]
    //}
  //  for(int i = 0; i < 10; i++){
  //	printf("errors_ordered[%d] size = %zu\n", i, errors_ordered[i].number);
  //  }
...
pattern_t find_patterns(corpus_t corpus, error_t error){
 ...
 fflush(stdout);
    ...
    int next2[number];
    int next3[number];
    printf("Now trying to find patterns\n");
